...
| 一键替换 | 开启后在「重复图片」区显示「替换为云端 / 替换为本地」按钮，一键合并重复组、自动更新笔记引用（默认关闭） |
...
3. 开启「自动上传」开关（保存设置时自动同步到 WebDAV；旧的「启用 WebDAV 同步」开关已合并进此开关）
4. 点击「测试连接」验证；如需用远端配置覆盖本地，点击「覆盖本地」
...
> ⚠️ 安全提示：同步到 WebDAV 的内容为解密后的**明文**，第三方 WebDAV 服务端会留存未加密的凭据/数据明文。请确保 WebDAV 服务端可信、启用 HTTPS 传输加密，并仅在可信网络下使用；不要将敏感 vault 同步到不可信的 WebDAV。
